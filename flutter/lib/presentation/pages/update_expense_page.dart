...
class UpdateExpensePage extends StatefulWidget {
  ...
  const UpdateExpensePage({Key? key, required this.tripId, required this.expense, this.forcePrepaid = false}) : super(key: key);
...
class _UpdateExpensePageState extends State<UpdateExpensePage> {
  ...
  void initState() {
    ...
    _isPrepaid = e.isPrepaid;
    _selectedParticipantIds.addAll(e.participantIds);
  ...
  void didChangeDependencies() {
    ...
    for (final id in provider.participantsMap.keys) {
      if (!_shareControllers.containsKey(id)) {
        final share = widget.expense.participantShares[id];
        _shareControllers[id] = TextEditingController(
          text: share != null ? share.toStringAsFixed(2) : '',
                ...
                child: Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      const Icon(Icons.edit_outlined, color: Colors.white70, size: 22),
                      const SizedBox(width: 8),
                      Expanded(child: Text(l10n.editExpense, style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold, letterSpacing: -0.3), overflow: TextOverflow.ellipsis)),
                    ],
                  ),
                ),
              ),
                                ...
                                children: [
                                  ...
                                  Column(
                                    ...
                                    children: [
                                      ...
                                      Text(l10n.prepaidDescription, style: const TextStyle(fontSize: 12, color: AppTheme.textSecondary)),
                                    ],
                                  ),
                                  const Spacer(),
